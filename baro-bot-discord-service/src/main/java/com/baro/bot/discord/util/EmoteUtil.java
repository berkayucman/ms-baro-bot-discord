...
public class EmoteUtil {
    ...
    public Emote getBooleanEmote(boolean val) {
        Guild guild = bot.getJda().getGuildById(Statics.GUILD_PALADIN_ID);
        if (val)
    ...
    public String getEmojiAsString(String name) {
        ...
        try {
            result = bot.getJda().getGuildById(Statics.GUILD_PALADIN_ID).getEmotesByName(name, false).get(0).getAsMention() + " ";
            return result;
        } catch (Exception ex) {
    ...
    public Emote getEmote(String name) {
        Emote result = null;
        try {
            result = bot.getJda().getGuildById(Statics.GUILD_PALADIN_ID).getEmotesByName(name, false).get(0);
            return result;
